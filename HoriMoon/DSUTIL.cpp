...
IDirectSoundBuffer *DSLoadSoundBuffer(IDirectSound *pDS, LPCTSTR lpName) {
	...
	dsBD.dwFlags = DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLFREQUENCY | DSBCAPS_LOCSOFTWARE;
	...
	else if (DSGetWaveFile(NULL, lpName, &dsBD.lpwfxFormat, &pbWaveData, &dsBD.dwBufferBytes, &pvBase)) {
		if (SUCCEEDED(IDirectSound_CreateSoundBuffer(pDS, &dsBD, &pDSB, NULL))) {
			if (!DSFillSoundBuffer(pDSB, pbWaveData, dsBD.dwBufferBytes)) {
				IDirectSoundBuffer_Release(pDSB);
				pDSB = NULL;
...
BOOL DSGetWaveResource(HMODULE hModule, LPCTSTR lpName, WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData, DWORD *pcbWaveSize) {
	...
	if (((hResInfo = FindResource(hModule, lpName, c_szWAV)) != NULL) && ((hResData = LoadResource(hModule, hResInfo)) != NULL) &&
		((pvRes = LockResource(hResData)) != NULL) && DSParseWaveResource(pvRes, ppWaveHeader, ppbWaveData, pcbWaveSize))
		return TRUE;

	return FALSE;
}

BOOL DSGetWaveFile(HMODULE hModule, LPCTSTR lpName, WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData, DWORD *pcbWaveSize, void **ppvBase) {
	void *pvRes;
	HANDLE hFile, hMapping;

	*ppvBase = NULL;

	hFile = CreateFile(lpName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
SNDOBJ *SndObjCreate(IDirectSound *pDS, LPCTSTR lpName, int iConcurrent) {
	...
	if (fFound) {
		...
		if ((pSO = (SNDOBJ*)LocalAlloc(LPTR, sizeof(SNDOBJ) + (iConcurrent - 1) * sizeof(IDirectSoundBuffer*))) != NULL) {
			int i;
...
IDirectSoundBuffer *SndObjGetFreeBuffer(SNDOBJ *pSO) {
	...
	if (pDSB = pSO->Buffers[pSO->iCurrent]) {
		...
		if (pDSB && (dwStatus & DSBSTATUS_BUFFERLOST))
			if (FAILED(IDirectSoundBuffer_Restore(pDSB)) || !DSFillSoundBuffer(pDSB, pSO->pbWaveData, pSO->cbWaveSize))
				pDSB = NULL;
	}
	return pDSB;
}
